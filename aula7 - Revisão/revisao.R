...
for (i in quantitativas) {
  resultado_summarize <- Summarize(satisfacao[[i]])
...
for (i in qualitativas) {
  resultado_qualitativo <-  prop.table(table(satisfacao[[i]]))
  cat("Resumo da variável", i, ":\n")
...
# d
plot_ly(satisfacao, x =~idade, type = 'histogram')

# 2
# a 
set.seed(42)
amostra <- satisfacao[sample(nrow(satisfacao), 120),replace= FALSE]
amostra
